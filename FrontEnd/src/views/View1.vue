    ...
    <div class="container">
      <div class="row">
        <div class="col-sm-12 text-center">
            ...
            <div v-if="IsASK">
                <button class="button-31" role="button" @click="WriteBetterSentence" > Write A Better Sentence</button>
            </div>
            <div v-if="IsBetterSentence">
                <textarea v-model="BetterSentence">
                </textarea>
            </div>
            <div>
...
export default {
  data() {
    return {
      ...
      Q_AList :[],
      BetterSentence:'',
      IsBetterSentence :false,
      items:["AO","QUAN","TUI","SACH"]
    }
  },
  computed: {
      reversedArray() {
  ...
  methods:{
    ddeleteItem:index=>{
      this.ỉtems.splice(index, 1);
      
    },
    AskChatGPT(){
      if(this.Question.length>0){
        this.$emit('loadingevent')
        this.IsBetterSentence = false;
        this.IsASK = false;
        setTimeout(this.callgrammarAPI,3000);
    }
    },
    async callgrammarAPI(){
        const linkURL = "http://127.0.0.1:8000/chatGPT/grammar?recv_data="
        try {
    ...
    WriteBetterSentence(){
      this.$emit('loadingevent')
      setTimeout(this.callbetterSentenceAPI,3000);

    },
    async callbetterSentenceAPI(){
      const linkURL = "http://127.0.0.1:8000/chatGPT/same-meaning?recv_data=";
      const oldSentence = this.Answer.substring(9);
      console.log(oldSentence);
      try {
          const response1 = await axios.get(linkURL+this.Question);
          this.BetterSentence = "BETTER ANSWER:" + response1.data[0].answer;
          this.IsBetterSentence = true;
          this.$emit('loadingevent')
        } catch (error) {
          console.error(error);
        }
